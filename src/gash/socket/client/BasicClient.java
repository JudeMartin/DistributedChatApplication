...
public class BasicClient {
	...
	public void stopSession() {
		...
		try {
			...
			byte[] msg = builder.encode(MessageBuilder.MessageType.leave, name,
					null, null).getBytes();
	...
	public void join(String name) {
		...
		try {
			MessageBuilder builder = new MessageBuilder();
			byte[] msg = builder.encode(MessageBuilder.MessageType.join, name,
					null, null).getBytes();
			socket.getOutputStream().write(msg);
			socket.getOutputStream().flush();
		} catch (Exception e) {
	...
	public void sendMessage(String message) {
		...
		try {
			MessageBuilder builder = new MessageBuilder();
			byte[] msg = builder.encode(MessageBuilder.MessageType.msg, name,
					message, null).getBytes();
			int T = 1000;
			while (T-- > 0) {
				/*
				 * The while loop is used to call the write the message T number
				 * of times
				 */
				System.out.println(" In the sendMessage() " + T); /* Outputs to the client */
				socket.getOutputStream().write(msg); /* writes to the server */
				socket.getOutputStream().flush();
			}
		} catch (Exception e) {
			e.printStackTrace();
